...
export type AppPage = 'dashboard' | 'budgets' | 'accounts' | 'members' | 'import' | 'transactions' | 'assign' | 'quick-add' | 'email-settings';
...
export function parseHashLocation(hash: string): { page: AppPage; search: string } {
  ...
  const validPages: AppPage[] = ['dashboard', 'budgets', 'accounts', 'members', 'import', 'transactions', 'assign', 'quick-add', 'email-settings'];
